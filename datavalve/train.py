...
# wandb is optional. Set WANDB_MODE=offline to enable logging.
...
def train(attn_implementation=None):
    ...
    if local_rank == 0 or local_rank == -1:
        if HAS_WANDB and training_args.report_to and "wandb" in training_args.report_to:
            try:
                ...
                print(f"[Rank 0] Initialized wandb (mode={wandb_mode})")
                if wandb_mode == "offline":
                    print("[Rank 0] Note: Run 'wandb sync' to upload offline runs.")
            except Exception as e:
                print(f"[Rank 0] Error initializing wandb: {e}")
                print("[Rank 0]           wandb")
    ...
    if training_args.use_clustering:
        ...
        if cluster_ids_path is None:
            rank0_print("[DataValve] No clustering results found, proceeding without diversity loss")
    ...
    if training_args.lora_enable:
        ...
        if local_rank == 0 or local_rank == -1:
            ...
            llava_config_fields = {
                # Vision Tower   
                'mm_vision_tower': getattr(model_args, 'vision_tower', './llava-ckpt/clip-vit-large-patch14'),
