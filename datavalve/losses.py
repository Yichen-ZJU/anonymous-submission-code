...
class DataValveLoss(nn.Module):
    ...
    def __init__(
        ...
        eps: float = 1e-8,
        loss_type_sand: str = "mean",  # "mean" or "cvar"
        loss_type_div: str = "entropy",  # "entropy" or "dpp"
        ...
        # [ICML Enhancement]       
        self.loss_type_sand = loss_type_sand
        self.loss_type_div = loss_type_div
        ...
        self.reward_group_enable = False
        self.group_norm = False
        self.reward_group_n_min = reward_group_n_min
        self.reward_group_clip = reward_group_clip
        self.reward_group_ema_decay = reward_group_ema_decay
        self.reward_group_fallback_min_active = reward_group_fallback_min_active
        ...
        if loss_type_sand == "cvar":
            self.cvar_sand_loss = CVaRSandLoss(alpha=cvar_alpha)
        if loss_type_div == "dpp":
            self.dpp_diversity_loss = DPPDiversityLoss(epsilon=dpp_epsilon)
        ...
        self.group_ema = {group_name: 0.0 for group_name in self.group_names}
        self.group_sq_ema = {group_name: 0.0 for group_name in self.group_names}
        self.group_initialized = {group_name: False for group_name in self.group_names}
        self.group_update_count = {group_name: 0 for group_name in self.group_names}
    ...
    def forward(
        ...
        loss_dict = {
            ...
            "entropy": rl_aux["entropy"].detach(),
            "golden_shard_id": rl_aux.get("golden_shard_id", zero).detach(),
            "ig_reward": rl_aux["ig_reward"].detach(),
            "ig_delta_norm": rl_aux.get("delta_norm", zero).detach(),
            "ig_log_psi_gamma": rl_aux.get("log_psi_gamma", zero).detach(),
            "ig_log_psi_z": rl_aux.get("log_psi_z", zero).detach(),
            "ig_gate_ready": rl_aux.get("gate_ready", zero).detach(),
        }
