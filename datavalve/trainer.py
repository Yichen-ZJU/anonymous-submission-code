...
from .losses import DataValveLoss, compute_selection_statistics
from .hook import ZeROGradientHookMixin


class DataValveValveTrainer(ZeROGradientHookMixin):
    """
    ...
    def __init__(
        ...
        lambda_sand: float = 1.0,
        lambda_diversity: float = 0.5,
        lambda_reinforce: float = 0.1,
        num_clusters: int = 20,
        ema_decay: float = 0.99,
        loss_type_sand: str = "mean",
        loss_type_div: str = "entropy",
        cvar_alpha: float = 0.5,
        dpp_epsilon: float = 1e-4,
        norm_sand: float = 1.0,
        norm_div: float = 0.1,
        norm_rl: float = 1.0,  # [2026-01-23   ]   0.1     1.0
        ...
        reward_group_enable: bool = False,
        group_norm: bool = False,
        reward_group_n_min: int = 2,
        reward_group_clip: float = 0.3,
        reward_group_ema_decay: float = 0.9,
        reward_group_fallback_min_active: int = 2,
        advantage_norm_floor: float = 0.05,
        reward_activity_ema_decay: float = 0.9,
        diversity_min_scale: float = 0.2,
        diversity_reward_activity_target: float = 0.05,
        diversity_reward_activity_power: float = 1.0,
        ig_z_clip: float = 3.0,
        device: str = 'cuda',
        ...
        self.diversity_min_scale = diversity_min_scale
        self.diversity_reward_activity_target = diversity_reward_activity_target
        self.diversity_reward_activity_power = diversity_reward_activity_power
        ...
        self.loss_fn = DataValveLoss(
            lambda_sand=lambda_sand,
            lambda_diversity=lambda_diversity,
            lambda_reinforce=lambda_reinforce,
            num_clusters=num_clusters,
            ema_decay=ema_decay,
            loss_type_sand=loss_type_sand,
            loss_type_div=loss_type_div,
            cvar_alpha=cvar_alpha,
            dpp_epsilon=dpp_epsilon,
            ...
            reward_group_enable=reward_group_enable,
            group_norm=group_norm,
            reward_group_n_min=reward_group_n_min,
            reward_group_clip=reward_group_clip,
            reward_group_ema_decay=reward_group_ema_decay,
            reward_group_fallback_min_active=reward_group_fallback_min_active,
            advantage_norm_floor=advantage_norm_floor,
            reward_activity_ema_decay=reward_activity_ema_decay,
            ig_z_clip=ig_z_clip,
        )
        ...
        self.source_to_group = SOURCE_TO_GROUP
        self.golden_source_counter = Counter()
        self.golden_group_counter = Counter()
        ...
        print(f"   _sand: {lambda_sand}, type: {loss_type_sand}")
        if loss_type_sand == "cvar":
            print(f"       CVaR alpha: {cvar_alpha} (      {cvar_alpha*100}%)")
        print(f"   _diversity: {lambda_diversity}, type: {loss_type_div}")
        if loss_type_div == "dpp":
            print(f"       DPP epsilon: {dpp_epsilon}")
        ...
        print(f"  diversity_min_scale: {diversity_min_scale}")
        print(f"  diversity_reward_activity_target: {diversity_reward_activity_target}")
        print(f"  diversity_reward_activity_power: {diversity_reward_activity_power}")
        if checkpoint_dir:
    ...
    def compute_router_loss_from_batch(
        ...
        [ICML Enhancement]    DPP diversity loss      img/txt features

                 DataLoader       Collator   worker       
        Router      batch dict               valve_trainer   
        ...
        Args:
            ...
            clip_distance: CLIP    (      )
            cluster_ids:    ID (       )
            img_features: CLIP image features (   DPP diversity loss)
            txt_features: CLIP text features (   DPP diversity loss)
    ...
    def export_reward_group_state(self) -> Dict[str, Any]:
        return self.loss_fn.export_reward_group_state()

    def load_reward_group_state(self, state: Optional[Dict[str, Any]]):
        self.loss_fn.load_reward_group_state(state)
    ...
    def log_golden_source_distribution(self, source_count_dict: Dict[str, int]):
        if not source_count_dict:
            return
        self.golden_source_counter.update(source_count_dict)
        group_counter = Counter()
        for source_name, count in source_count_dict.items():
            group_name = self.source_to_group.get(str(source_name))
            if group_name is not None:
                group_counter[group_name] += int(count)
        self.golden_group_counter.update(group_counter)

    def get_golden_distribution_stats(self) -> Dict[str, Dict[str, float]]:
        return {
            'source_ratio': self.summarize_counts(self.golden_source_counter),
            'group_ratio': self.summarize_counts(self.golden_group_counter),
        }
    ...
    def _save_checkpoint(self):
        ...
        data = {
            ...
            'total_sampled': self.total_sampled,
            'statistics': stats,
            'reward_group_state': self.export_reward_group_state(),
            'golden_distribution_stats': self.get_golden_distribution_stats(),
        }
    ...
    def load_reward_group_state_from_checkpoint(self, checkpoint_path: str):
        if not checkpoint_path or not os.path.exists(checkpoint_path):
            return
        try:
            with open(checkpoint_path, 'r') as f:
                data = json.load(f)
            self.load_reward_group_state(data.get('reward_group_state'))
            print(f"[DataValveValveTrainer]     reward_group_state: {checkpoint_path}")
        except Exception as e:
            print(f"[Warning]    reward_group_state   : {e}")
...
def get_modality_length_grouped_indices_for_superbatch(lengths, batch_size, world_size, generator=None):
    ...
    # [2026-03-09 CRITICAL FIX]      generator     None 
...
class DataValveSuperBatchSampler(Sampler):
    ...
    def _compute_stage_params(self):
        ...
        if self.router_selected > 0 and self.router_samples > 0:
            ...
            #      router_batches        super_batch_size 
            #      router_samples      
...
class DataValveValveCollator:
    ...
    def __init__(
        ...
        Args:
            ...
            world_size:            
        """
        #    [FIX]     valve_trainer    DeepSpeed NVTX          
        # Collator   worker             valve_trainer
        ...
        #    [CRITICAL FIX]    warmup_batches           
        #     bug     total_samples       Collator    warmup    4151 batch
    ...
    def __call__(self, batch_candidates: List[Dict]) -> Dict[str, torch.Tensor]:
        ...
        #    [CRITICAL FIX]    total_batches        valve_trainer.current_batch
        # Collator   worker               current_batch
...
class LLaVATrainer_DataValve(LLaVATrainer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # [Batch C FIX]        CUDA           
        self.debug_cuda_sync = os.environ.get("DJY_DEBUG_CUDA_SYNC", "0") == "1"
        ...
        # [Bug2 FIX]           reward(t)    action(t-k)
        #   1   maxlen=1                router_update_interval   
        queue_maxlen_env = os.environ.get("DJY_ACTION_QUEUE_MAXLEN", "1")
        try:
    ...
    def compute_golden_loss(self, retain_graph: bool = True, golden_dataloader: Optional[DataLoader] = None, shard_id: int = -1) -> Dict[str, Any]:
        ...
        # [2026-03-16   ]      training       model.eval()        Dropout
        ...
        try:
            ...
            if not math.isfinite(val_loss_value):
                print(f"[DIAG] compute_golden_loss NaN/Inf detected! "
                      f"total_loss={total_loss}, total_batches_run={total_batches_run}, "
                      f"rank={rank}, world_size={world_size}, "
                      f"num_batches_total={num_batches_total}, start_batch={start_batch}, end_batch={end_batch}")
            if rank == 0 and (self.state.global_step % 10 == 0 or self.state.global_step <= 5):
                self.valve_trainer.log_golden_source_distribution(source_count_local)
                dist_stats = self.valve_trainer.get_golden_distribution_stats()
                print(f"[Golden Groups] step={self.state.global_step}, counts={group_count_local}, losses={group_loss_dict}")
                print(f"[Golden Source Ratio] step={self.state.global_step}, ratios={dist_stats['source_ratio']}")
                print(f"[Golden Group Ratio] step={self.state.global_step}, ratios={dist_stats['group_ratio']}")
            return {
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        =====================================================
           ...
           Router     L_sand + L_div + L_reinforce   
            L_router =  1 * L_sand +  2 * L_div +  3 * L_reinforce
        ...
        #    [CRITICAL FIX]      Collator          batch   
        # Collator   worker             batch      GlobalStep    
        # GlobalStep             max_steps                 Collator    
        ...
        if self.state.max_steps > 0:
            ...
            if warmup_batches and warmup_batches > 0:
                ...
                #             Collator    
                if router_is_warmup != is_warmup_by_step and self.state.global_step % 20 == 0:
                    print(f"[DEBUG] Warmup    : Collator={router_is_warmup}, GlobalStep={is_warmup_by_step} "
                          f"(step={self.state.global_step}/{warmup_steps}, batches={warmup_batches})")
                    print(f"       Collator         batch       ")

        #   [DEBUG]         10 step       rank 
        should_log_compute_loss = True
        try:
            import torch.distributed as dist
            if dist.is_initialized():
                should_log_compute_loss = (dist.get_rank() == 0)
        except Exception:
            pass

        if should_log_compute_loss and (self.state.global_step % 10 == 0 or self.state.global_step <= 5):
            print(f"[compute_loss] step={self.state.global_step}, "
                  f"router_is_warmup={router_is_warmup}, "
                  f"has_candidate_indices={router_candidate_indices is not None}, "
                  f"has_selected_indices={router_selected_indices is not None}, "
                  f"num_candidates={router_num_candidates}")

        #                Router                  
        router_has_all_candidates_debug = inputs.get("router_has_all_candidates", False)
        if should_log_compute_loss and (self.state.global_step % 10 == 0 or self.state.global_step <= 5):
            print(f"[router_has_all_candidates] step={self.state.global_step}, value={router_has_all_candidates_debug}")
        ...
        #    [CRITICAL FIX] Router            Router              LLaVA collate     GPU
            ...
            if router_has_all_candidates and (router_candidate_samples is not None or router_candidate_dataset_indices is not None):
                ...
                try:
                    ...
                    if should_log_compute_loss and (self.state.global_step % 10 == 0 or self.state.global_step <= 5):
                        ...
                        print(f"[Router Selection] step={self.state.global_step}, candidates={len(indices_list)}, selected={selected_count}, mask_sum={router_mask_for_training.sum().item()}")
        ...
        # LLaVA                
        if labels is not None:
            #    [CRITICAL]      HuggingFace     loss     LLaVA    
        ...
        # ============================================================
        # [ICML Logic] Zero-Shock Handover Schedule
        #           (Curriculum Learning)
        # Phase 1 (Prior): Router              DPP + CVaR 
        # Phase 2 (Data): Router    RL            
        #          (Convex Combination)             
        # ============================================================
        if self.state.max_steps > 0 and not router_is_warmup:
            # A.       
            current_step = self.state.global_step
            total_steps = self.state.max_steps

            #    warmup   
            warmup_ratio = self.valve_trainer.warmup_ratio
            ...
            if current_step < warmup_steps:
                ...
                router_steps_total = total_steps - warmup_steps
                if router_steps_total > 0:
                    p_eff = (current_step - warmup_steps) / router_steps_total
                else:
                    p_eff = 1.0
            ...
            # C.      (Gradient Normalizers)
            #           Loss          
            NORM_SAND = self.valve_trainer.config.norm_sand  # CVaR Loss       0.5~1.0
            NORM_DIV  = self.valve_trainer.config.norm_div   # LogDet Loss         
            NORM_RL   = self.valve_trainer.config.norm_rl    # RL Loss        

            # D.               (Linear Complementary Handover)
            # [FIX]        base lambda     base=0       
            # valve_trainer.lambda_sand/diversity              
            base_lambda_sand = self.valve_trainer.lambda_sand
            base_lambda_div  = self.valve_trainer.lambda_diversity

            # Sand (Anchor):    base_lambda_sand      
            current_lambda_sand = NORM_SAND if base_lambda_sand > 0 else 0.0

            # Div:    base_lambda_div       Full-min:        reward activity gate
            # - progress_scale:                    
            # - activity_gate: reward         DPP reward                   
            reward_activity = None
            if hasattr(self.valve_trainer.loss_fn, 'reward_activity_ema'):
                try:
                    reward_activity = float(self.valve_trainer.loss_fn.reward_activity_ema.item())
                except Exception:
                    reward_activity = None
            if reward_activity is None or not np.isfinite(reward_activity):
                reward_activity = float(self.valve_trainer.diversity_reward_activity_target)

            if base_lambda_div > 0:
                progress_scale = self.valve_trainer.diversity_min_scale + (1.0 - self.valve_trainer.diversity_min_scale) * (1.0 - p_eff)
                reward_ratio = reward_activity / max(self.valve_trainer.diversity_reward_activity_target, 1e-8)
                reward_ratio = max(0.0, reward_ratio)
                activity_gate = min(1.0, reward_ratio ** self.valve_trainer.diversity_reward_activity_power)
                current_lambda_div = NORM_DIV * progress_scale * activity_gate
            else:
                progress_scale = 0.0
                activity_gate = 0.0
                current_lambda_div = 0.0

            # RL (Emerging Intelligence):                   
            current_lambda_rl   = NORM_RL * p_eff

            # E.     
            self.valve_trainer.loss_fn.lambda_sand = current_lambda_sand
            self.valve_trainer.loss_fn.lambda_diversity = current_lambda_div
            self.valve_trainer.loss_fn.lambda_reinforce = current_lambda_rl

            # F.                 100   
            #    _current_router_loss_dict       on_log callback        wandb 
            if not hasattr(self, '_current_router_loss_dict'):
                self._current_router_loss_dict = {}

            self._current_router_loss_dict.update({
                "datavalve/progress_effective": p_eff,
                "datavalve/lambda_sand": current_lambda_sand,
                "datavalve/lambda_div": current_lambda_div,
                "datavalve/lambda_div_progress_scale": progress_scale,
                "datavalve/lambda_div_activity_gate": activity_gate,
                "datavalve/lambda_div_reward_activity": reward_activity,
                "datavalve/lambda_rl": current_lambda_rl,
            })
        # ============================================================
    ...
    def compute_router_loss_after_backward(
        ...
        try:
            ...
            #    [CRITICAL]    detach CLIP         LLaVA     
            #                                 detach
            img_feats = img_feats.detach().to(device=router_device, dtype=router_dtype)
            txt_feats = txt_feats.detach().to(device=router_device, dtype=router_dtype)

            # [A!=B FIX]      compute_loss        A router_mask_for_training 
            #    A                          RL         
            ...
            # [2026-04-07 FIX]          micro_step   should_update      rank          collective   
            is_main_rank = True
            try:
                import torch.distributed as dist
                if dist.is_initialized():
    ...
    def training_step(self, model, inputs):
        ...
        # V20:   /   hook    LoRA       Router update slot    capture
        ...
        if global_can_train_router:
            ...
            if router_loss is not None and router_loss.requires_grad:
                ...
                if global_nonfinite:
                    if local_nonfinite:
                        print(f"[Warning] step={self.state.global_step} router_loss     local       Router backward/step")
                        try:
                            diag_items = {}
                            for k in [
                                'loss_val',
                                ...
                                if k in router_loss_dict:
                                    v = router_loss_dict[k]
                                    if isinstance(v, torch.Tensor):
                                        diag_items[k] = float(v.detach().float().item())
                                    else:
                                        diag_items[k] = float(v)
                            print(f"[DIAG] step={self.state.global_step} router_loss nonfinite components: {diag_items}")
                        except Exception as e:
                            print(f"[Warning] step={self.state.global_step}         : {e}")
                    else:
                        print(f"[Warning] step={self.state.global_step}    rank    router_loss     global       Router backward/step")
                    router_loss = None
            ...
            if router_loss is not None and router_loss.requires_grad:
                ...
                if self.router_optimizer is not None and should_step_router_optimizer:
                    ...
                    # [2026-03-22 FIX]    T_max     step    Cosine       
        ...
        #    [CRITICAL FIX]    update            compute_loss      lambda   
        if not hasattr(self, '_current_router_loss_dict'):
            self._current_router_loss_dict = {}
        ...
        if all_datasets:
            ...
            self._current_router_loss_dict['datavalve/selected_dataset_entropy'] = float(selected_entropy)
            self._current_router_loss_dict['datavalve/dataset_enrich_min'] = float(enrich_min)
            self._current_router_loss_dict['datavalve/dataset_enrich_max'] = float(enrich_max)
            self._current_router_loss_dict['datavalve/dataset_enrich_mean'] = float(enrich_mean)
        self._current_router_loss_dict.update({
            "train/llava_loss": llava_loss.item() if isinstance(llava_loss, torch.Tensor) else llava_loss,
            "datavalve/warmup_done": not router_is_warmup,
            "datavalve/router_steps": self.router_step_count,
            "datavalve/router_optimizer_steps": self.router_optimizer_step_count,
        })
        if isinstance(router_info, dict):
            ...
            self._current_router_loss_dict['datavalve/candidate_batch_size'] = float(len(candidate_indices))
            self._current_router_loss_dict['datavalve/selected_batch_size'] = float(len(selected_indices))
            if len(candidate_indices) > 0:
                self._current_router_loss_dict['datavalve/effective_selection_ratio'] = float(len(selected_indices)) / float(len(candidate_indices))
            for metric_name in (
            ...
            if topk_margin is not None and np.isfinite(float(topk_margin)):
                ...
                self._current_router_loss_dict['datavalve/selection_topk_margin_ema'] = float(self.valve_trainer.selection_topk_margin_ema)
        ...
        if selected_seen_total > 0:
            ...
            self._current_router_loss_dict['datavalve/selected_unique_count'] = selected_seen_unique
            self._current_router_loss_dict['datavalve/selected_duplicate_rate'] = 1.0 - selected_seen_unique / max(selected_seen_total, 1.0)
            dataset_len = 0
            try:
                ...
                dataset_len = 0
            if dataset_len > 0:
                self._current_router_loss_dict['datavalve/selected_coverage_ratio'] = selected_seen_unique / float(dataset_len)
        if torch.cuda.is_available():
            try:
                ...
                self._current_router_loss_dict['datavalve/gpu_mem_allocated_gb'] = float(torch.cuda.memory_allocated(device_idx)) / (1024.0 ** 3)
                self._current_router_loss_dict['datavalve/gpu_mem_reserved_gb'] = float(torch.cuda.memory_reserved(device_idx)) / (1024.0 ** 3)
            except Exception:
                pass
        self._current_router_loss_dict['datavalve/router_update_progress'] = float(self.router_accum_step_count) / max(float(self.router_optimizer_accum_steps), 1.0)
        ...
        if router_loss_dict:
            ...
            #    Warmup      0
            self._current_router_loss_dict.update({
                "datavalve/loss_total": 0.0,
                "datavalve/loss_val": 0.0,
                "datavalve/loss_sand": 0.0,
                "datavalve/loss_diversity": 0.0,
                "datavalve/loss_reinforce": 0.0,
                "datavalve/advantage": 0.0,
                "datavalve/val_loss_ema": 0.0,
                "datavalve/selection_rate": 0.0,
            })
...
class DataValveWandbCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        if trainer is not None and hasattr(trainer, "_current_router_loss_dict"):
            ...
            if loss_dict:
                #    [CRITICAL]      wandb.log()     HuggingFace    
                try:
                    wandb.log(loss_dict)
                except Exception as e:
