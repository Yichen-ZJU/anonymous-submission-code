...
> Two auxiliary scripts are prepared for the camera-ready release:
>
...
> **On logging.** WandB is disabled by default (`--report_to none`).
> To enable experiment tracking, install `wandb` separately and set
> `--report_to wandb` with `WANDB_MODE=offline` (or `online`).
